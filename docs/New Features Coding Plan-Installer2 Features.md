...
- Do not add `--kexec` flags; keep `KEXEC_BOOT` as the expert override and use `nxd convert` for software takeovers.
...
- Do not run for `--flake github`, `--flake tea`, or custom non-local flake refs.
- Do not run for hosts with `buildSystem = false` (metadata-only/verification targets).
...
- [x] **Lockfile Archive**
  - [x] Add local-source archive eligibility helper.
  - [x] Implement `hosts/<host>/flake.lock` copy helper.
  - [x] Wire successful local-source switch.
  - [x] Wire successful local-source deploy, including batch hosts.
  - [x] Add tests for local source, non-local source, missing root lockfile, and destination path rendering.

- [x] **GitHub Token Support**
  - [x] Add global CLI/runtime option.
  - [x] Add env fallback.
  - [x] Add optional `gh auth token` fallback if appropriate.
  - [x] Add Nix option rendering with redaction support.
  - [x] Thread options through eval, archive/materialization, local build, remote builder, target-native, and target-instantiated flows.
  - [x] Add tests proving debug/rendered logs redact the token.

- [x] **Cross Strategy**
  - [x] Expose optional `cross` metadata in `deploymentHosts` evaluation.
  - [x] Deserialize optional `cross` metadata in Rust.
  - [x] Add `BuildStrategy::Cross`.
  - [x] Parse `--build-on cross`.
  - [x] Validate NixOS host and configured cross metadata before building.
  - [x] Render `crossNixosConfigurations.<host>.<attr>` from `NixBuilder::target_attr`.
  - [x] Wire the strategy through `build_attribute`.
  - [x] Add command-rendering tests for cross toplevel and Disko attrs.

- [x] **Proxmox Static IP Bootstrap**
  - [x] Add Proxmox `net0`, `net1`, and nested `bootstrap` options.
  - [x] Preserve `deployment.proxmox.network` as `net0` during host migration.
  - [x] Validate `bootstrap.interface` against configured NICs.
  - [x] Update VM creation to emit optional `net1`.
  - [x] Make IP discovery use the configured bootstrap interface.
  - [x] Implement provider-local `qm guest exec` static IP/VLAN helper.
  - [x] Add optional `bootstrap.gateway` handling without inferred default gateway.
  - [x] Add plan-time custom ISO validation for static bootstrap hosts.
  - [x] Block official ISO fallback for static bootstrap hosts.
  - [x] Add tests for command rendering, interface mapping, fallback order, and ISO blocking.

---

## 4. Verification Checklist

- [x] `cd apps/installer-rs && cargo fmt --check`
- [x] `cd apps/installer-rs && cargo test`
- [x] `cd apps/installer-rs && cargo clippy --all-targets -- -D warnings`
- [x] `nix eval .#deploymentHosts.gaming.cross`
- [x] `nix eval .#crossNixosConfigurations.gaming.config.system.build.toplevel.drvPath`
- [x] `nix run .#installer-rs -- switch -t gaming --action build --build-on cross -d`
- [x] Successful local-source switch archives `flake.lock` to `hosts/<host>/flake.lock`.
- [x] Non-local repo source does not write `hosts/<host>/flake.lock`.
- [x] GitHub token is accepted and redacted in debug output.
- [x] Default kexec `auto` still bypasses live NixOS installer environments and takes over persistent OS targets.
- [x] `nxd convert` successfully installs the requested final host configuration in-place.
- [x] Proxmox two-NIC hosts can provision `net1` and discover/reach the bootstrap IP through the configured interface.
