...
use crate::progress::log::{normalize_multiline_message, ProgressEvent, StatusLevel};
...
impl LogTarget {
    ...
    fn write_event(&mut self, level: StatusLevel, message: &str, file_message: &str) {
        ...
        let file_lines: Vec<&str> = if file_message.is_empty() {
            vec![""]
        } else {
            file_message.lines().collect()
        };
