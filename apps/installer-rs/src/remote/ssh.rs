...
impl SshOptions {
    ...
    pub fn ssh_args_before_target(&self) -> Vec<String> {
        ...
        if let Some(ref proxy) = self.proxy_jump {
            args.push("-o".to_string());
            args.push(format!("ProxyCommand=ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o LogLevel=ERROR -W %h:%p {}", proxy));
        }
    ...
    pub fn run(
        ...
        match mode {
            ...
            SshOutputMode::LoggerStreamed { logger } => {
                ...
                let stdout_handle =
                    crate::process::stream_output_lines(stdout, logger.clone(), false);
