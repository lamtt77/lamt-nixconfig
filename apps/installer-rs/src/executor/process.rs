...
impl CommandExecutor {
    ...
    pub fn execute(
        ...
        if program == "nix" || program == "rsync" {
            ...
            for arg in args {
                let clean_arg = if arg.starts_with("ssh://") {
                    &arg[6..]
                } else {
                    *arg
                };
        ...
        let stderr_handle = stream_output_lines(stderr, logger.clone(), true);
    ...
    pub fn execute_with_stdin(
        ...
        let safe_stdin = if stdin_content.contains("PRIVATE KEY") {
            "<private key hidden>"
        } else {
            stdin_content
        };
...
mod tests {
    ...
    fn test_command_recording_and_mocks() {
        ...
        let out_substring =
            CommandExecutor::execute("qm", &["status", "101"], log.clone()).unwrap();
    ...
    fn log_status_does_not_infer_level_from_message_text() {
        ...
        assert_eq!(
            content,
            "[INFO] SUCCESS Target host SSH key staged successfully.\n"
        );
    }
}
