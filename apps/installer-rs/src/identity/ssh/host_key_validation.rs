use super::host_key_mismatch::{resolve_host_key_mismatch, HostKeyMismatch};
...
pub fn validate_and_sync_target_host_key(
    ...
    let local_key_file = secrets_repo
        .join("hosts")
        .join(&ctx.hostname)
    ...
    crate::info!(
        &logger,
        "Checking target host SSH key connection for {}...",
        ctx.hostname
    );
    let get_pubkey_cmd = "sudo cat /etc/ssh/ssh_host_ed25519_key.pub 2>/dev/null; echo '---HOSTNAME---'; hostname 2>/dev/null";

    let (active_pub_key, remote_hostname) = read_target_host_identity(&ssh_target, get_pubkey_cmd);
    validate_remote_hostname(ctx, remote_hostname.as_deref())?;
...
fn read_target_host_identity(
    ...
    match crate::remote::ssh::probe_stdout(ssh_target, get_pubkey_cmd, 3) {
        Some(stdout) => {
            ...
            let resolved_hostname = if hostname_part.is_empty() {
                None
            } else {
...
fn resolve_missing_local_key_choice(
    ...
    crate::info!(
        &logger,
        "How would you like to resolve the missing local host key?"
    );
    crate::info!(
        &logger,
        "  1) Import active target key into secrets repo (Target -> Secrets)"
    );
