...
pub fn handle_kexec_takeover(
        ...
        match CommandExecutor::execute_ssh(initial_ssh, check_cmd, logger.clone()) {
            Ok(output) => {
                ...
                if cleaned == "NIXOS_LIVE" {
                    ...
                    if let Ok(fs_type) =
                        CommandExecutor::execute_ssh(initial_ssh, fs_cmd, logger.clone())
                    {
    ...
    info!(
        logger,
        "Target requires in-memory boot transition (Kexec). Preparing..."
    );
    ...
    if low_mem {
        info!(
            logger,
            "Tuning target memory parameters for in-memory boot..."
        );
        let sudo_prefix = if initial_ssh.starts_with("root@") {
            ""
        } else {
            "sudo -n "
        };
    ...
    let run_prefix = if initial_ssh.starts_with("root@") {
        ""
    } else {
        "sudo -n "
    };
