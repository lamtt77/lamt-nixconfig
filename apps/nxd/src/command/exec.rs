...
pub async fn execute_command(
	...
	let escaped_parts: Vec<String> =
		command.iter().map(|arg| crate::process::shell_escape(arg)).collect();
	...
	let semaphore =
		if parallel > 0 { Some(Arc::new(tokio::sync::Semaphore::new(parallel))) } else { None };
	...
	for ctx in targets {
		...
		let handle = task::spawn_blocking(move || -> Result<(), String> {
			...
			let mode = if stream {
				...
				let hash = hostname_for_thread.chars().map(|c| c as usize).sum::<usize>();
				let color_code = colors[hash % colors.len()];
				let prefix =
					color::colorize(&format!("[{}]", hostname_for_thread), color_code, ColorMode::Auto);
			...
			let res =
				session.run(&cmd_str, mode).map_err(|e| format!("Failed to run ssh command: {}", e))?;
	...
	if !failed.is_empty() {
		let list: Vec<String> = failed.iter().map(|(h, e)| format!("{}: {}", h, e)).collect();
