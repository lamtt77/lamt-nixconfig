...
use super::{HostPlan, OperationKind, OperationRisk, ProviderKind};

impl HostPlan {
	pub fn render(&self, ctx: &RuntimeContext) {
		...
		let strategy_name =
			self.build_strategy.as_ref().map(|s| s.label()).unwrap_or_else(|| "N/A".to_string());

		println!("Host: {}", self.hostname);
		match self.operation {
			OperationKind::Deploy => {
				if ctx.deployment.wsl.enable {
					println!("  Mode:        WSL import and converge");
				} else {
				...
				if ctx.deployment.wsl.enable {
					println!(
						"  Control SSH: {}@{}",
						ctx.deployment.wsl.windows_user, ctx.deployment.wsl.windows_host
					);
					println!("  Distribution: {}", ctx.deployment.wsl.distribution);
					println!("  Install Root: {}", ctx.deployment.wsl.install_root);
					println!("  Guest Target: {}", ctx.target_ip);
					println!("  Bootstrap:   direct from selected producer");
				} else {
				...
				if let Some(ref reason) = self.skip_reason {
					...
					let action = if ctx.deployment.wsl.enable {
						if self.risk == OperationRisk::High {
							"unregister WSL distribution, import bootstrap artifact, and converge"
						} else if self.provider_state == Some(crate::providers::ProviderState::Missing) {
							"build/reuse bootstrap artifact, import WSL distribution, and converge"
						} else {
							"reuse WSL distribution and converge final NixOS configuration"
						}
					} else {
						match self.risk {
			...
			OperationKind::Switch => {
				...
				println!("  Provider:    {}", provider_name);
				println!("  Build On:    {}", strategy_name);
				let action = self.switch_action.as_deref().unwrap_or("switch").to_string();
				println!("  Action:      {}", action);
			}
		}
...
pub fn provider_label(kind: Option<ProviderKind>) -> &'static str {
	match kind {
		None => "None (Standard)",
		Some(ProviderKind::Proxmox) => "Proxmox VM",
		Some(ProviderKind::Vmware) => "VMware VM",
		Some(ProviderKind::DigitalOcean) => "DigitalOcean",
		Some(ProviderKind::Wsl) => "WSL",
	}
}
...
pub fn confirm_batch(plans: &[HostPlan], force: bool) -> Result<bool, Box<dyn std::error::Error>> {
	...
	if has_skipped {
		println!("Warning: The following hosts will be SKIPPED by default: {:?}", skipped_hosts);
		println!(
	...
	let prompt = if plans.len() > 1 {
		format!("Are you sure you want to proceed with batch {}?", operation_name)
	} else {
		format!("Are you sure you want to proceed with {}?", operation_name)
	};
...
mod tests {
	...
	fn renders_provider_labels() {
		assert_eq!(provider_label(None), "None (Standard)");
		assert_eq!(provider_label(Some(ProviderKind::Proxmox)), "Proxmox VM");
		assert_eq!(provider_label(Some(ProviderKind::Vmware)), "VMware VM");
		assert_eq!(provider_label(Some(ProviderKind::DigitalOcean)), "DigitalOcean");
	}
}
