...
pub fn validate_interface(
	...
	if fs::metadata(&sys_path).is_err() {
	...
	if !has_ip_address(interface, listen_ip)? {
		return Err(
			format!("IP address '{}' is not configured on interface '{}'", listen_ip, interface).into(),
		);
	}
...
fn has_ip_address(interface: &str, ip: &str) -> Result<bool, Box<dyn std::error::Error>> {
	let output = Command::new("ip").args(["addr", "show", "dev", interface]).output()?;

	if !output.status.success() {
		return Err(
			format!("Failed to execute 'ip addr' command: {}", String::from_utf8_lossy(&output.stderr))
				.into(),
		);
	}
