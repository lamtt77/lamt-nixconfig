...
pub fn resolve_host_key_mismatch(
	...
	crate::warn!(&logger, "Target host '{}' SSH key mismatch detected!", mismatch.ctx.hostname);
	crate::info!(&logger, "  Local key (in lamt-secrets): {}", mismatch.local_pub_key);
	crate::info!(&logger, "  Target key (active on host): {}", mismatch.active_pub_key);
	...
	match choice.as_str() {
		...
		"3" => {
			crate::warn!(&logger, "Proceeding anyway. Secrets decryption may fail on target.");
		}
		_ => {
...
fn resolve_mismatch_choice(logger: Logger) -> String {
	...
	if !runtime_options.force {
		...
		crate::info!(&logger, "  1) Overwrite target key to match secrets (Secrets -> Target)");
		crate::info!(&logger, "  2) Update secrets to match target key (Target -> Secrets)");
...
fn overwrite_target_key(
	...
	crate::info!(&logger, "Updating SSH host key on target to match local key...");
...
fn update_secrets_key(
	...
	crate::info!(&logger, "Updating secrets repository with target host key...");
	...
	fs::write(mismatch.local_pub_file, format!("{}\n", mismatch.active_pub_key))?;
