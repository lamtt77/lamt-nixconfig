...
impl DeploymentMode {
	pub fn from_context(ctx: &RuntimeContext, convert: bool) -> Self {
		if convert {
		...
		} else if !ctx.deployment.proxmox.cloud_init.image.is_empty() {
			let user = if ctx.deployment.proxmox.cloud_init.user.is_empty() {
				"ubuntu".to_string()
			} else {
				ctx.deployment.proxmox.cloud_init.user.clone()
			...
			Self::NixosInstall { initial_user: "root".to_string() }
		}
	}
	...
	pub fn is_destructive(&self) -> bool {
		matches!(self, Self::NixosInstall { .. } | Self::CloudInitConvert { .. })
	}
...
mod tests {
	...
	#[test]
	fn deployment_mode_converts_existing_vm_without_cloud_init_image() {
		let ctx = context_with_cloud_init("", "abc");
		let mode = DeploymentMode::from_context(&ctx, true);

		assert_eq!(mode.label(), "Cloud-Init Convert to NixOS");
		assert_eq!(mode.initial_user(), "abc");
		assert!(mode.is_destructive());
		assert!(mode.uses_nix_build());
	}
}
