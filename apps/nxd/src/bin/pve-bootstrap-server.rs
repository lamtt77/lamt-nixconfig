...
fn resolve_pve_answer_server_path() -> Result<PathBuf, Box<dyn std::error::Error>> {
	// Check path first
	if let Some(output) =
		Command::new("which").arg("pve-answer-server").output().ok().filter(|o| o.status.success())
	{
	...
	let output = Command::new("nix")
		.args(["build", ".#pve-answer-server", "--no-link", "--print-out-paths"])
		.output()?;

	if !output.status.success() {
		return Err(
			format!("Failed to resolve pve-answer-server: {}", String::from_utf8_lossy(&output.stderr))
				.into(),
		);
	}
...
fn build_assets(target: &str, listen_ip: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
	...
	if !output.status.success() {
		return Err(
			format!("Failed to build PXE assets: {}", String::from_utf8_lossy(&output.stderr)).into(),
		);
	}
