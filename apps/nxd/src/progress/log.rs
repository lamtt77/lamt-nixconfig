	...
	lines
		...
		.map(|line| {
			if line.trim().is_empty() {
				...
				line.char_indices().nth(common_indent).map(|(idx, _)| &line[idx..]).unwrap_or("")
			}
		})
		.collect::<Vec<_>>()
		.join("\n")
...
impl<'a> ProgressEvent<'a> {
	pub fn new(level: StatusLevel, host: Option<&'a str>, message: impl Into<String>) -> Self {
		Self { level, host, message: message.into() }
	}

	pub fn render(&self, color_mode: ColorMode) -> String {
		let prefix = if let Some(host) = self.host { format!("[{}] ", host) } else { "".to_string() };
...
mod tests {
	...
	fn normalizes_multiline_message_indentation() {
		...
		assert_eq!(normalized, "Timing breakdown:\n  Context loading: 1s\n  Execution: 2s");
	}

	#[test]
	fn preserves_single_line_indentation() {
		assert_eq!(normalize_multiline_message("  already aligned"), "  already aligned");
	}
}
