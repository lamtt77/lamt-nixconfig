...
impl Provider for VmwareProvider {
	...
	fn resource_identity(&self) -> String {
		format!("VMX {}", self.vmx_path)
	}

	fn inspect(&self) -> Result<ProviderState, Box<dyn std::error::Error>> {
		if self.vmx_path.is_empty() || !Path::new(&self.vmx_path).exists() {
			Ok(ProviderState::Missing)
		} else if self.is_running() {
			Ok(ProviderState::Running)
		} else {
			Ok(ProviderState::Present)
		}
	}
	...
	fn create(&self) -> Result<(), Box<dyn std::error::Error>> {
		...
		let vmx_dir =
			vmx_path.parent().ok_or_else(|| format!("Invalid VMX path: {}", self.vmx_path))?;
		let vm_name = vmx_path.file_stem().and_then(|s| s.to_str()).unwrap_or(&self.hostname);

		// Determine CPU architecture
		let target_arch = if self.system.contains("x86_64") { "x86_64" } else { "aarch64" };
		...
		let custom_iso_name =
			format!("nixos-minimal-{}-{}-linux.iso", crate::config::nixos_iso_version(), target_arch);
		...
		if let Some(workspace_iso) = crate::config::find_custom_iso(target_arch) {
			info!(self.logger, "Found workspace own-built NixOS ISO: {}", workspace_iso.display());
			...
			if !target_dest.exists() {
				...
				if let Err(e) = fs::copy(&workspace_iso, &target_dest) {
					warn!(self.logger, "Warning: Failed to copy own-built ISO from workspace: {:?}", e);
				}
			}
			if target_dest.exists() {
		...
		if local_iso.is_none() && custom_iso.exists() {
			info!(self.logger, "Own-built custom NixOS ISO found: {}", custom_iso.display());
			local_iso = Some(custom_iso);
		}

		// 3. Fallback to official ISO name in default ISO dir
		if local_iso.is_none() && official_iso.exists() {
			info!(self.logger, "Official NixOS ISO found: {}", official_iso.display());
			local_iso = Some(official_iso.clone());
		}

		// 4. Wildcard fallback search in default ISO dir
		if let Some(entries) = fs::read_dir(iso_dir).ok().filter(|_| local_iso.is_none()) {
			for entry in entries.flatten() {
				let path = entry.path();
				if let Some(filename) = path.file_name().and_then(|s| s.to_str()).filter(|_| path.is_file())
				{
					let match_custom = filename.starts_with("nixos-minimal-")
						&& filename.contains(target_arch)
						&& filename.ends_with(".iso");
					let match_generic = filename.contains(target_arch) && filename.ends_with(".iso");
		...
		info!(self.logger, "NixOS ISO selected: {}", selected_iso.display());
		...
		let env_disk_size = env::var("VM_DISK_SIZE").unwrap_or_else(|_| {
			if self.disk_size.is_empty() { "50".to_string() } else { self.disk_size.clone() }
		});
		...
		let manager_args =
			["-c", "-s", &size_arg, "-a", "lsilogic", "-t", "0", &disk_path.to_string_lossy()];
		...
		info!(self.logger, "Generating VMX configuration file at {}...", self.vmx_path);
		...
		info!(self.logger, "VMware VMX configuration generated successfully.");

		info!(self.logger, "Starting VMware VM from VMX path: {}...", self.vmx_path);
	...
	fn destroy(&self) -> Result<(), Box<dyn std::error::Error>> {
		...
		if let Some(parent) = vmx_path.parent().filter(|p| p.exists()) {
	...
	fn get_ip(&self, poll: bool) -> Result<String, Box<dyn std::error::Error>> {
		...
		let mac = parse_mac_from_vmx(&self.vmx_path).ok_or_else(|| {
			format!("Failed to read ethernet MAC address from VMX file at {}", self.vmx_path)
		})?;

		info!(self.logger, "Locating DHCP lease for MAC address: {}...", mac);
...
fn generate_vmx_content(
	...
	let guest_os = if arch == "x86_64" { "other6xlinux-64" } else { "arm-other6xlinux-64" };
	let net_dev = if arch == "x86_64" { "e1000e" } else { "vmxnet3" };
	...
	if let Ok(entries) = fs::read_dir(lease_dir) {
		for entry in entries.flatten() {
			...
			if path
				.file_name()
				.and_then(|n| n.to_str())
				.filter(|n| n.starts_with("vmnet-dhcpd-") && n.ends_with(".leases"))
				.is_some()
			{
