...
use crate::process::{Logger, shell_escape};
...
pub fn nix_copy_command(builder: &NixBuilder, copy_target: &str, source: &str) -> String {
	let substitute_flag =
		if builder.substitute_on_destination { " --substitute-on-destination" } else { "" };

	let mut options = crate::remote::ssh::SshOptions::nix_copy();
	if let Some(proxy) = crate::context::find_proxy_jump_for_ip(&builder.target_ip) {
		let proxy_cmd = format!(
			"ssh -o {} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -W %h:%p {}",
			crate::remote::ssh::WARN_WEAK_CRYPTO_OPTION,
			proxy
		);
		options = options.with_proxy_command(Some(proxy_cmd));
	}
