...
let
  ...
  currentSystem = if isDarwin
    then "/nix/var/nix/profiles/system"
  ...
  diffScript = ''
    if [ -e "${currentSystem}" ]; then
      ...
      if [ -n "$TARGET_SYSTEM" ]; then
        ...
        ${pkgs.nvd}/bin/nvd --nix-bin-dir=${if config.nix.enable then "${config.nix.package}/bin" else "/nix/var/nix/profiles/default/bin"} diff "$OLD_SYSTEM" "$TARGET_SYSTEM" || true
